...
customers.map(function (item, UK) {
        ...
        tdCity.innerHTML = item.address.city;
        ...
        let phone = (item.address.phone).replace(/\D/g, "")
...
console.log('2) kaç adet customer var? ', customers.length);

//3) a ( küçük veya büyük harf) ile başlayan customer ları console a yaz
let filteredCustomers2 = customers.filter(q => q.contactName.startsWith('A'));
console.log('A harfi ile başlayanlar ', filteredCustomers2);

//4) Country 'UK' olan customerları console a yaz
let UKcustomers = customers.filter(q => q.address.country === "UK");
console.log(UKcustomers);

//5) customer ları companyName e göre sırala ( sort )
let sortedCustomers = customers.sort((a,b) =>
a.companyName.localeCompare(b.companyName));
console.log('Sort list',sortedCustomers);


//6) id si "DUMON" olan customer ı console a yaz! tek bir object
let customer = customers.find(q => q.id === 'DUMON');
console.log('id si DUMON olan customer' , customer);

// Phone format 7) 8)

console.log("7) *** Telefon numarası 1 ile başlayanları console a yaz", customers.filter(q => (q.address.phone).replace(/\D/g, "").startsWith('1')));
console.log("8) ***** Telefon numaralarını console a istenilen formatta yaz", phoneList)